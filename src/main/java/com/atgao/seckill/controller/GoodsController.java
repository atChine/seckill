...
import com.atgao.seckill.vo.DetailVo;
...
public class GoodsController {
	...
	@ResponseBody
	public String toList(Model model, SysUser user,
	...
	@ResponseBody
	public String toDetail2(Model model, SysUser user, @PathVariable Long goodsId,
	                        HttpServletRequest request, HttpServletResponse response) {
		ValueOperations valueOperations = redisTemplate.opsForValue();
		//Redis中获取页面，如果不为空，直接返回页面
	...
	@ResponseBody
	public RespBean toDetail(SysUser user, @PathVariable Long goodsId) {
		GoodsVo goodsVo = goodsService.findGoodsVoByGoodsId(goodsId);
		Date startDate = goodsVo.getStartDate();
		Date endDate = goodsVo.getEndDate();
		Date nowDate = new Date();
